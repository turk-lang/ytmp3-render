    ...
    for order in orders:
        try:
            ...
            with YoutubeDL(opts_probe) as y1:
                ...
                if info.get("is_live"): raise DownloadError("Canlı yayın desteklenmiyor.")
                fmt = choose_format(info)
            opts_dl = dict(opts_probe)
            opts_dl["format"] = fmt
            before = set(os.listdir(DOWNLOAD_DIR))
            with YoutubeDL(opts_dl) as y2: y2.download([url])
            after = set(os.listdir(DOWNLOAD_DIR))
            new_files = sorted(after - before, key=lambda f: os.path.getmtime(os.path.join(DOWNLOAD_DIR,f)), reverse=True)
            if new_files: return new_files[0]
            return f"{info.get('title','audio')}.{ 'mp3' if ffmpeg_available() else info.get('ext','m4a') }"
        except Exception as e:
            last_err = e
    raise RuntimeError(str(last_err) if last_err else "Bilinmeyen hata")
...
def index():
    ...
    if request.method == "POST":
        ...
        if file and file.filename: file.save("/tmp/cookies.txt")
        try:
