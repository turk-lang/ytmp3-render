...
- Başarılı indirmeden sonra direkt ana form sayfasına geri döner (hiç mesaj/buton göstermez)
"""
...
def build_opts(*, player_clients: str, cookiefile: Optional[str] = None,
    ...
    assert isinstance(player_clients, str)
    opts: Dict[str, Any] = {
        ...
        "extractor_args": {"youtube": {"player_client": player_clients, "skip": ["configs"]}},
...
def run_download(url: str) -> str:
    ...
    # Cookie varsa sadece WEB client; yoksa mobil->web dene
    if cookie:
    ...
    if ("sign in to confirm you're not a bot" in low) or ("bot olmadığınızı" in low):
        msg += "\nİpucu: cookies.txt'yi güncelleyin ve/veya YTDLP_PROXY kullanın."
...
def index():
    if request.method == "POST":
        ...
        try:
            _ = run_download(url)
            # BAŞARILI İNDİRME: hiçbir mesaj/buton göstermeden forma geri dön
